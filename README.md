...
5. Run the desired entry point:
   ...
   - For gRPC:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Catalogs.gRpc
      ```

   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Catalogs.AsyncWorker
      ```
...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-catalogs-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Catalogs.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-catalogs-rest ms-catalogs-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-catalogs-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Catalogs.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-catalogs-grpc ms-catalogs-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-catalogs-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Catalogs.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-catalogs-worker ms-catalogs-worker
   ```
